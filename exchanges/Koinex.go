...
func GetKoinexPrice(c *models.Client, price *controllers.BitcoinPrice) {
	...
	if(e != nil) {
		...
		if(len(rsp.Buy_orders.Data) > 0 && len(rsp.Sell_orders.Data) > 0) {
			price.KoinexBuyPrice,_ = strconv.ParseFloat(rsp.Buy_orders.Data[0].Price_per_unit,64);
			price.KoinexSellPrice,_ = strconv.ParseFloat(rsp.Sell_orders.Data[0].Price_per_unit,64);
		}
	}
}